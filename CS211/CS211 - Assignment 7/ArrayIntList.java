 ...
 * This program completes the three assignments for assignment 7 (upToNowTotal, isPairSorted, and removeLast). 
 **/
...
// YOUR CODE GOES HERE
    public ArrayIntList upToNowTotal() {  
    	ArrayIntList list = new ArrayIntList();    // creates a new ArrayIntList
    	int dataSize = this.size() - 1;    // creates variable for size of the array
    	for (int i = 0; i <= dataSize; i++){  // loops through the array
    		if (i == 0){    // if the first in the array
    			list.add(elementData[i]);  // then add element to the array list
    		} else { 
    			int prevData = list.get(i - 1);  // create temp. variable for previous data point (prevData)
    			int currData = elementData[i];  // create temp. variable for current data point (currData)
    			list.add(prevData + currData);  // add prevData and currData together and add it to the list
    		}
    	}
    ...
    public boolean isPairSorted() {
    	int dataSize = this.size() - 1;    // creates variable for size of the array
    	for (int i = 1; i <= dataSize; i = i + 2){  // loops through the array
    		if (elementData[i - 1] > elementData[i]){  // checks to see if the prior array data is larger than the current, if it is...
    ...
    public void removeLast(int n) {
    	int dataSize = this.size() - 1;    // creates variable for size of the array
    	for (int i = dataSize; i > dataSize - n; i--){   // loops through the array (from the end to the end minus the amount provided by the client)
    		this.remove(i);  // removes the value 
    	}
    }    
}
